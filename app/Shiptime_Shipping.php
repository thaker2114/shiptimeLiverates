...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
        ...
        if ( ! class_exists( 'ShipTime_Shipping_Method' ) ) {
            class ShipTime_Shipping_Method extends WC_Shipping_Method {
                ...
                public function __construct() {
                    ...
                    $this->id                 = 'shiptime'; 
                    $this->method_title       = __( 'ShipTime Shipping', 'shiptime' );  
                    $this->method_description = __( 'Custom Shipping Method for ShipTime', 'shiptime' );  
                   ...
                   if(empty($setApikeys['ApiKey'])) {
                    ...
                    $this->form_fields = array(
                     ...
                     'enabled' => array(
                          ...
                          'type' => 'checkbox',
                          'description' => __( 'Enable this checkbox if you want to connect store.', 'shiptime' ),
                          'default' => 'no'
                          ),
                     'connect' => array(
    ...
    function add_shiptime_shipping_method( $methods ) {
     ...
     add_action('wp_ajax_oauth_es', 'oauth_es_callback');

    function oauth_es_callback()
    {
        $getApidata = get_option('woocommerce_shiptime_settings');
        $Apidata = [];
        $Apidata['storeUrl'] = site_url();
        $Apidata['apiSecret'] = $getApidata['ApiKey'];
        $Apidata['platform'] = 'WoocommerceNativeApi';
    ...
    function oauth_action_button_st()
    {
        wp_enqueue_script(
            'oauth_action_button_st',
            plugin_dir_url(__FILE__) . 'includes/assets/js/admin/ajax_oauth_es.js',
            array('jquery'),
            '5.0.4');
    }
   ...
   if($shipTimeValues)
  {
   ...
   function enableLiverateUrl($req)
     {    
      $response = array();
      $response['liverateUrl'] = $req['liverateUrl'];
      ...
      if(!empty($res))
      {
        ...
        $seturls = get_option( 'woocommerce_shiptime_settings' );
        $seturls['liverateUrl'] = $response['liverateUrl'];
    ...
    function storeDelete($req)
     {    
      ...
      $response = array();
      $response['apiKey'] = $getApiKey['apiKey'];
      $res = new WP_REST_Response($response);
      $setApikeys = get_option( 'woocommerce_shiptime_settings' );
